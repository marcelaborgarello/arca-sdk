 ...
 */
export async function callArcaApi(url: string, options: CallApiOptions): Promise<Response> {
    ...
    if (isNode) {
        return new Promise((resolve, reject) => {
            ...
            const agent = new https.Agent({
                ...
                minVersion: 'TLSv1',
                // @ts-expect-error - Propiedad específica para mitigar "dh key too small" en Node 18+
                minDHSize: 1024,
            ...
            req.on('error', (error: any) => {
                ...
                if (message.includes('dh key too small')) {
                    message =
                        'Error SSL de ARCA (DH Key too small). Verifique su versión de OpenSSL/Node.';
                }
                reject(
                    new ArcaNetworkError(
                        `Error de red al comunicarse con ARCA (HTTPS): ${message}`,
                        {
                            url,
                            originalError: error,
                        },
            ...
            req.on('timeout', () => {
                ...
                reject(
                    new ArcaNetworkError(
                        `Tiempo de espera agotado (${timeout}ms) al conectar con ARCA: ${url}`,
                    ),
                );
            });
